...
def build_splits(df, seed=42):
    ...
    save_split(splits["in_domain"], safe_name("Domain"))

    for key, split in splits["loto"].items():
        save_split(split, safe_name(f"loto_{key}"))

    for key, split in splits["loco"].items():
        save_split(split, safe_name(f"loco_{key}"))

    save_split(splits["external"], safe_name("Transfer"))
